                    ...
                    <div class="root-reply-container">
                        <div class="root-reply-avatar">
                            <div class="bili-avatar">
                                <img @click="$util.goUser(item.user_id)" style="width: 40px;height: 40px;"
                                    :src="$util.url.load + item.avatar" alt="">
                            </div>
                        </div>
                        <div class="content-wrap">
                            <div class="user-info">
                                <div class="user-name" @click="$util.goUser(item.user_id)">{{ item.user_name }}
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        async getComment() {
            let res
            // 获取评论数
            res = await this.$util.get('getCommentByvideo/' + this.$route.params.id)
            if (res.code != 10000) return
            this.commentsChange = this.$util.numChange(res.data.result.count)
            //获取评论
            ...
            for (let ele of this.commentList) {
                ...
                res = await this.$util.get('getUsers/' + ele.user_id)
                if (res.code != 10000) return
        ...
        async submitComment() {
            ...
            let res = await this.$util.post('users/addComment', { video_id: this.$route.params.id, user_id: this.$store.state.userInfo.id, talk: this.comment })
            this.comment = ''
            // token过期，重新登录
            if (res.code == 401) {
